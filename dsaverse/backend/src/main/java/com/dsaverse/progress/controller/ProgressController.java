...
public class ProgressController {
    ...
    public ResponseEntity<ApiResponse<UserQuestionAttempt>> submitAttempt(
            @PathVariable String slug,
            @Valid @RequestBody SubmissionRequest request,
            @CurrentUser UserPrincipal currentUser) {

        UserQuestionAttempt attempt = progressService.submitAttempt(currentUser.getId(), slug, request);
