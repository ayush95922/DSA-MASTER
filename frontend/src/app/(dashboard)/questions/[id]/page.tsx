...
import { useQuestionDetail, useQuestionEditorial, useSubmitAttempt, SubmissionPayload } from "@/hooks/use-questions";
...
export default function QuestionDetailPage({ params }: PageProps) {
  ...
  // Track whether code has been initialized for this slug
  ...
  // One-shot initialization: set starter template ONLY when the question
  // first loads for this slug. Subsequent language changes reset the code
  // only if the user hasn't modified it from a starter template.
  useEffect(() => {
    if (question && initializedForSlug.current !== slug) {
  ...
  // Reset code when language changes — only update if current code matches
  // ANY starter template (i.e. user hasn't typed custom code yet)
  const prevLanguage = useRef(selectedLanguage);
  useEffect(() => {
    if (prevLanguage.current !== selectedLanguage) {
